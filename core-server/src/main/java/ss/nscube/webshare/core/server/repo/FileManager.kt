...
class FileManager(val server: WebShareServer) {
    val mutex = Any()
    ...
    val selectedFiles: ArrayList<WebFile>
        get() {
            return files.filter {
                it.fileType == SelectionType.TypeSelected
    ...
    fun getReceivedFilesLength(): Long {
        var length = 0L
        for (file in files) if (file.fileType == SelectionType.TypeReceived) length += file.length
        return length
    }
...
//    }
    ...
    fun addReceived(file: WebFile): Boolean {
        file.fileType = SelectionType.TypeReceived
        val result = add(file)
//        callOnUpdate()
        return result
    }
    ...
    fun addSelection(file: WebFile): Boolean {
        file.fileType = SelectionType.TypeSelected
        val result = add(file)
        callOnUpdate()
        return result
    }
    ...
    fun removeAllSelection() {
        ...
        for (file in files) if (file.fileType == SelectionType.TypeSelected) removeFiles.add(file)
