...
public final class QRCodeWriter {
    ...
    public Bitmap encode(Context context, String contents, Map<EncodeHintType, ?> hints) throws WriterException {
        float radiusFactor = 0.75f;
        int backgroundColor =  Color.TRANSPARENT;
        int color = UiUtil.Companion.primaryTextColor(context);
        ...
        int _300dp = (int) UiUtil.Companion.getInstance().dp(290);
        int outputWidth = Math.max(_300dp, qrWidth);
        int outputHeight = Math.max(_300dp, qrHeight);

        int multiple = Math.min(outputWidth / qrWidth, outputHeight / qrHeight);
//        Log.d("QR", "encode: " + multiple + " " + _300dp + " " + qrWidth + " "  + qrHeight + " " + outputWidth + " " + outputHeight + " ");
        int padding = 20;
        ...
        Log.d("QR", "imageBloks: " + imageBloks + " " + imageBlockX + " " + imageBloks * multiple + " " + imageSize);
        int imageX = (size - imageSize) / 2;
        if (includeSideQuads) {
            blackPaint.setColor(color);
            drawSideQuadsGradient(canvas, blackPaint, rect, sideQuadSize, multiple, padding, size, 1.1f, radii, backgroundColor, color);
        }
...
//            drawable.setTint(color);
    ...
    public static void drawSideQuads(Canvas canvas, float xOffset, float yOffset, Paint blackPaint, float sideQuadSize, float multiple, int padding, float size, float radiusFactor, float[] radii, boolean isTransparentBackground) {
        ...
        for (int a = 0; a < 3; a++) {
            ...
            float r;
            if (isTransparentBackground) {
                rectTmp.set(x + multiple, y + multiple, x + (sideQuadSize - 1) * multiple, y + (sideQuadSize - 1) * multiple);
                r = (sideQuadSize * multiple) / 4.0f * radiusFactor;
                clipPath.reset();
                clipPath.addRoundRect(rectTmp, r, r, Path.Direction.CW);
                clipPath.close();
                canvas.save();
                canvas.clipPath(clipPath, Region.Op.DIFFERENCE);
            }
            r = (sideQuadSize * multiple) / 3.0f * radiusFactor;
            rectTmp.set(x, y, x + sideQuadSize * multiple, y + sideQuadSize * multiple);
            canvas.drawRoundRect(rectTmp, r, r, blackPaint);
            if (isTransparentBackground) {
                canvas.restore();
            }

            r = ((sideQuadSize - 2) * multiple) / 4.0f * radiusFactor;
            rectTmp.set(x + multiple * 2, y + multiple * 2, x + (sideQuadSize - 2) * multiple, y + (sideQuadSize - 2) * multiple);
            canvas.drawRoundRect(rectTmp, r, r, blackPaint);
        }
    }
